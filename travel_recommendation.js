...
function displayResults(results) {
	...
	if (results.length === 0) {
		...
		results.forEach((result) => {
			...
			resultDiv.innerHTML = `
                <div class="p-4 mb-8 bg-white rounded-lg shadow flex flex-col items-center">
                    <h2 class="text-xl text-black font-bold mb-2">${result.name}</h2>
                    <img class="object-cover mb-2 rounded-lg" src="${result.imageUrl}" alt="${result.name}" style="width: 320px; height: 180px;">
